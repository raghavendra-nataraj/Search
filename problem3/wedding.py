...
e. Cost Function = The cost function does not matter much here since the goal is to arrange X number people on minimum number of tables, i.e., Y such that each person on the table knows no other person on the same table. The cost function keeps increasing with each person added to the table until the goal state is reached. Thus, we can say that the cost is constant.
'''
